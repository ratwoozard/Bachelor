...
- SoluTalent-proces: `staging_imported` -> `matched`
...
```mermaid
flowchart TD
    problemstilling[Problemstilling_og_PF] --> ontologi[Ontologisk_niveau]
    ontologi --> epistemologi[Epistemologisk_niveau]
    epistemologi --> metodologi[Metodologisk_niveau]
    metodologi --> metodeTeknisk[Metode_teknisk_niveau]
    metodeTeknisk --> empiri[Empiri_og_datakilder]
    empiri --> analyseLogik[Triangulering_og_analyseLogik]
    analyseLogik --> kvalitet[Etik_og_kvalitetskriterier]
```
