...
flowchart TB
  subgraph ONTO["Ontologisk niveau"]
    O1["Genstandsfelt: observerbare hændelser + social praksis"]
  end
  subgraph EPIST["Epistemologisk niveau"]
    E1["Interpretivisme / moderne hermeneutik → interviews"]
    E2["Positivistisk inspireret målingsspor → systemudtræk"]
  end
  subgraph METH["Metodologisk niveau"]
    M1["Pragmatisk ramme"]
    M2["Abduktion + sekventielt udforskende design"]
    M3["Indlejret single-case: organisatorisk delmængde"]
  end
  subgraph MET["Metode / teknisk niveau"]
    T1["Interviews via purposive sampling"]
    T2["Artefaktanalyse af platformen"]
    T3["Systemdata via tværsnit"]
    T4["Triangulering: designet flow vs. faktisk adfærd"]
  end
  ONTO --> EPIST --> METH --> MET
```
